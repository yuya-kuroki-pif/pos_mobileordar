# POS レジ + モバイルオーダー + 管理ダッシュボード

飲食店向けのシステムです。2 つのアプリで構成されています。

```
apps/
  dashboard/   本部・店舗向け管理ダッシュボード（:3000）
  terminal/    店舗端末: POSレジ / キッチンディスプレイ / モバイルオーダー（:3001）
supabase/      両者が共有するスキーマ・マイグレーション
docs/          機能差分と実装状況
```

- **店舗端末** — 卓の QR から客が注文し、厨房のディスプレイに流れ、レジで会計するまで。
- **管理ダッシュボード** — マスター設定・集計・分析・CRM。
  dinii セルフダッシュボード互換の開発指示書に沿って構築中です
  （進捗は [docs/spec-progress.md](docs/spec-progress.md)）。

```bash
npm install
npm run dev            # ダッシュボード
npm run dev:terminal   # 店舗端末
```
